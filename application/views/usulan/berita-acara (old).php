...
<body style="font-family: calibri; font-size: 11pt">
	...
	<table width="100%">
		<tr>
			<td>
				<center>
					<h3>BERITA ACARA SEMINAR <?= strtoupper($usulan[0]->nm_mk) ?> (<?= acronym($usulan[0]->nm_mk) ?>)</h3>
				</center>
	    	</td>
		</tr>
	</table>
	...
	<table border="0" width="100%">
		<tr>
			<td style="text-align: justify; line-height: 1.5">
				...
				Pada hari ini, <?= $hari[date('w', strtotime($penjadwalan->tanggal))] ?> tanggal <?= date_indo($penjadwalan->tanggal)  ?> pukul <?= substr($penjadwalan->mulai, 0, 5)  ?> WIB sampai dengan <?= substr($penjadwalan->selesai, 0, 5)  ?> WIB telah dilaksanakan Seminar <?= ucwords(strtolower($usulan[0]->nm_mk)) ?> Mahasiswa Tahun Akademik <?= explode(' ', $_SESSION['nama_smt'])[0] ?>, Program Studi <?=  $detail->nama_prodi ?>, menyatakan bahwa :
			</td>
		</tr>
	</table>
	...
	<table border="0" width="100%" cellpadding="2">
	  ...
	  <tr>
	    <td width="240">DINYATAKAN</td>
 	    <td>: LULUS / <?= $data == 1 ? '<del>TIDAK LULUS</del>' : 'TIDAK LULUS' ?>  *)</td>
	  </tr>
	  <tr>
	    <td>DENGAN NILAI</td>
	    <td>: <u>&nbsp;&nbsp;&nbsp;&nbsp; <?= $nilai_akhir = ceil(($nilai_pemb + $nilai_peng + $nilai_ket) / 3) ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u></td>
	  </tr>
	  <tr>
	    <td>NILAI HURUF</td>
	    <td>: <u>&nbsp;&nbsp;&nbsp;&nbsp; <?= nilai_mutu($nilai_akhir) ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u></td>
	  </tr>
	  <tr>
	  	<td colspan="2">
